...
func (h *ScooterStatsHandler) GetShiftStatsHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		response.RespondWithError(w, http.StatusInternalServerError, "Failed to connect to scooter database")
		return
	}
	defer func() {
	...
	if err := botDB.Ping(); err != nil {
		...
		response.RespondWithError(w, http.StatusInternalServerError, "Failed to ping scooter database")
		return
	}
